...
impl<'a, 'b, A> Emitter<'a, 'b, A> {
    ...
    pub unsafe fn addk(&mut self, i: u32, pc: usize) -> usize {
        let base = self.get_base();
        let v1 = self.get_reg(base, i >> 7 + 8 + 1 & !(!(0u32) << 8));
        ...
        // Add floats.
        let n1 = self.load_num_as_float(v1, next, &[BlockArg::Value(base)]);
        let n2 = self.load_num_as_float(v2, next, &[BlockArg::Value(base)]);
        let t = self.fb.ins().iconst(I8, 3 | 1 << 4);
    ...
    pub unsafe fn div(&mut self, i: u32, pc: usize) -> usize {
        let base = self.get_base();
        let ra = self.get_reg(base, i >> 7 & !(!(0u32) << 8));
        let v1 = self.get_reg(base, i >> 7 + 8 + 1 & !(!(0u32) << 8));
        let v2 = self.get_reg(base, i >> 7 + 8 + 1 + 8 & !(!(0u32) << 8));
    ...
    pub unsafe fn band(&mut self, i: u32, pc: usize) -> usize {
        let base = self.get_base();
        let ra = self.get_reg(base, i >> 7 & !(!(0u32) << 8));
        let v1 = self.get_reg(base, i >> 7 + 8 + 1 & !(!(0u32) << 8));
        let v2 = self.get_reg(base, i >> 7 + 8 + 1 + 8 & !(!(0u32) << 8));
        ...
        let v = self.fb.ins().call(self.tointegerns, &[v1, out]);
        let v = self.fb.inst_results(v)[0];
        let load_converted = self.fb.create_block();
        ...
        self.fb
            .ins()
            .brif(v, load_converted, [], call_meta, &[BlockArg::Value(base)]);

        self.fb.switch_to_block(load_converted);
        self.fb.seal_block(load_converted);
        ...
        self.fb
            .ins()
            .jump(and, &[BlockArg::Value(i1), BlockArg::Value(v)]);
        ...
        // Perform AND.
        let skip = self.get_label(pc + 1);
        let &[i1, i2] = self.fb.block_params(and).as_array().unwrap();
        let t = self.fb.ins().iconst(I8, 3 | 0 << 4);
        let v = self.fb.ins().band(i1, i2);
    ...
    pub unsafe fn shl(&mut self, i: u32, pc: usize) -> usize {
        let base = self.get_base();
        let ra = self.get_reg(base, i >> 7 & !(!(0u32) << 8));
        let v1 = self.get_reg(base, i >> 7 + 8 + 1 & !(!(0u32) << 8));
        let v2 = self.get_reg(base, i >> 7 + 8 + 1 + 8 & !(!(0u32) << 8));
        ...
        let v = self.fb.ins().icmp_imm(IntCC::Equal, v, 3 | 0 << 4);
        let check_v2 = self.fb.create_block();
        let convert_v1 = self.fb.create_block();

        self.fb.append_block_param(check_v2, I64);

        self.fb
            .ins()
            .brif(v, check_v2, &[BlockArg::Value(i)], convert_v1, []);

        self.fb.switch_to_block(convert_v1);
        self.fb.seal_block(convert_v1);
        ...
        // Load converted value.
        let i = self.fb.ins().stack_load(
            I64,
            self.values[0],
            offset_of!(UnsafeValue<A>, value_.i) as i32,
        );

        self.fb.ins().jump(check_v2, &[BlockArg::Value(i)]);
        ...
        self.fb
            .ins()
            .jump(shift, &[BlockArg::Value(i1), BlockArg::Value(i2)]);
        ...
        let v = self.fb.ins().call(self.shift_l, &[i1, i2]);
        let v = self.fb.inst_results(v)[0];
        let t = self.fb.ins().iconst(I8, 3 | 0 << 4);
    ...
    pub unsafe fn lei(&mut self, i: u32, pc: usize) -> usize {
        let base = self.get_base();
        let ra = self.get_reg(base, i >> 7 & !(!(0u32) << 8));
        let im = (i >> 7 + 8 + 1 & !(!(0u32) << 8)) as i32 - ((1 << 8) - 1 >> 1);
        let tt = self.fb.ins().load(
        ...
        let v = self
            .fb
            .ins()
            .icmp_imm(IntCC::SignedLessThanOrEqual, lhs, i64::from(im));
        let check_res = self.fb.create_block();
        ...
        self.fb.ins().brif(v, cmp_float, [], call_mt, []);
        ...
        // Compare float.
        let rhs = self.fb.ins().f64const(im as f64);
        let v = self.fb.ins().fcmp(FloatCC::LessThanOrEqual, lhs, rhs);
    ...
    pub unsafe fn gei(&mut self, i: u32, pc: usize) -> usize {
        ...
        self.fb.switch_to_block(invoke_mt);
        self.fb.seal_block(invoke_mt);
        self.fb.set_cold_block(invoke_mt);
