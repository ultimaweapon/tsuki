...
use crate::{
    ContentType, Float, Nil, StackValue, Str, Table, Thread, UserData, luaH_finishset, luaH_get,
    luaH_getn,
};
use alloc::boxed::Box;
...
#[cfg_attr(not(feature = "jit"), path = "interp.rs")]
mod engine;
mod opcode;
...
unsafe fn l_strton<A>(obj: *const UnsafeValue<A>) -> Option<UnsafeValue<A>> {
    if !((*obj).tt_ as c_int & 0xf as c_int == 4 as c_int) {
        ...
        let st = (*obj).value_.gc.cast::<Str<A>>();
