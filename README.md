...
A call to async function without any suspend on Tsuki is faster than mlua about 2.6x. For 1 suspend Tsuki it faster about 2.3x. For 8 suspend Tsuki is faster about 1.8x.
...
- `string.pack`, `string.packsize` and `string.unpack` requires UTF-8 string for format string and format error always argument error.
- `string.unpack` always report data error as argument error.
...
No. LuaJIT is Tracing JIT while Tsuki is Method JIT. It is likely for LuaJIT to be faster since Lua is dynamic typing, which make all type checking still present in Method JIT. The only benefits Tsuki got from JIT is the elimination of some branches, interpreter jump table and dead code.
